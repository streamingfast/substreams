...
func (e *Engine) FunctionSignature(module *manifest.Module) (*FunctionSignature, error) {
	...
	case manifest.ModuleKindBlockIndex:
		return e.blockIndexFunctionSignature(module)
	default:
...
func (e *Engine) blockIndexFunctionSignature(module *manifest.Module) (*FunctionSignature, error) {
	arguments, err := e.ModuleArgument(module.Inputs)
	if err != nil {
		return nil, fmt.Errorf("generating MustModule intputs: %w", err)
	}
	...
	fn := NewFunctionSignature(module.Name, "blockIndex", maybeTranslateType(outType), module.UpdatePolicy, arguments)
