...
func init() {
	sink.AddFlagsToSet(estimateLocalCmd.Flags(),
	...
	estimateLocalCmd.Flags().Int("samples", 10, "Number of segments of 1000 blocks to estimate against")
	estimateLocalCmd.Flags().Int("parallel-requests", 1, "Number of parallel requests to perform")
	estimateLocalCmd.Flags().BoolP("yes", "y", false, "Skip confirmation prompt and proceed with estimation")

	rootCmd.AddCommand(estimateLocalCmd)
}

var estimateLocalCmd = &cobra.Command{
	Use:   "estimate-local [<manifest> [<module_name>]]",
	Short: "Estimate Substreams usage costs by sampling segments from this client",

	Long: cli.Dedent(`
		Estimates the cost (processed blocks and egress bytes) of running a Substreams module
		by streaming multiple 1000-block segments evenly distributed across the requested block
		...
		The command works by:
		  ...
		  2. Streaming exactly 1000 blocks from each segment (aligned to 1000-block boundaries)
		...
		Limitations and Warnings:
		  ...
		  - Every sampled block is streamed back, so the estimation itself costs egress
...
func estimateLocalE(cmd *cobra.Command, args []string) error {
