...
The `sink.type` defines the type of the config that we are expecting, in our case it's [sf.substreams.sink.sql.v1beta1.GenericService](https://buf.build/streamingfast/substreams-sink-sql/docs/main:sf.substreams.sink.sql.v1beta1#sf.substreams.sink.sql.v1beta1.GenericService) (click on the link to see the message definition).
...
The `sink.config.dsn` defined the connection details to your Postgres database, it requires the database IP address, username, and password, which depend on your PostgreSQL installation. Adjust `dev-node` to your own username (`postgres` is usually the default), `insecure-change-me-in-prod` to your password and `127.0.0.1:5432` to where your database can be reached.
...
Environment variable are expanded at runtime in `sink.config.dsn` meaning you can (and should) avoid inlining it in your config file.

The `substreams.dev.yaml` (and the packaged version if you pack it) should be treated as private material and proper management of it should be made.
...
```bash
substreams-sink-sql setup ./sink/substreams.dev.yaml
```
...
```bash
substreams-sink-sql run "mainnet.eth.streamingfast.io:443" ./sink/substreams.dev.yaml
```
