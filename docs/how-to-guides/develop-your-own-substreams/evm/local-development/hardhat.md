...
async function main() {
  ...
  const counter = await Counter.deploy(0); // Start with count = 0
...
```javascript
const Counter = await ethers.getContractFactory("Counter");
const counter = Counter.attach("YOUR_CONTRACT_ADDRESS_HERE");
await counter.getCount(); // Should return 2n
```
...
```bash
cd .. # Back to substreams-ethereum-local
mkdir substreams
cd substreams
```
...
### HardHat Deployment Errors

**Problem:** "insufficient funds for gas"
- The dev mode provides unlimited ETH to the default account
- Ensure you're using the correct network configuration

**Problem:** "nonce too high" or "nonce too low"
- Reset HardHat's cache: `npx hardhat clean`
