...
Optionally select **Expose a GraphQL API over this database** to deploy a read-only [GraphQL layer](./graphql-layer.md) alongside the sink. You can also add or remove it later from the sink's **Edit** dialog.
...
- [Serve your data over GraphQL](./graphql-layer.md) — add a read-only GraphQL API and console over the sink's database.
