...
12. `Subgraph feed mode`: there are two in which subgraphs can receive data from Substreams: _entities_ and _triggers_
    - **Entities**: all the logic lives in the Substreams, so you can't modify the subgraph code in AssemblyScript.
...
The [entities defined in the GraphQL schema](https://thegraph.com/docs/en/developing/creating-a-subgraph/#the-graphql-schema) are generated into AssemblyScript code by running the `graph codegen` command.
