...
- CLI: the `substreams-sink-sql` binary is now part of the `substreams` CLI. `substreams sink database-changes {run,setup,create-user,generate-csv,inject-csv,tools}` (alias `db-changes`) covers the DatabaseChanges mode (`db_out` modules with a `schema.sql`), and `substreams sink from-proto` covers the relational-mappings mode (arbitrary protobuf output). Differences from the standalone binary: the connection string moved from a positional argument to `--dsn` (or the `SUBSTREAMS_SINK_DSN` environment variable) on every subcommand, metrics are served via the standard `--prometheus-addr` sink flag (default `localhost:9102`), pprof is an opt-in `--pprof-listen-addr` flag, and `sink database-changes tools cursor delete <module_hash>` now deletes only the given module's cursor (the standalone binary deleted all of them due to a bug).
