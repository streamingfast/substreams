...
- Sink: `substreams sink postgres` in from-proto mode buffers rows on local disk in PostgreSQL's binary COPY wire format and loads them from a background goroutine, one segment per transaction. The stream no longer waits on the database, and blocks already downloaded survive a restart instead of being streamed — and paid for — twice. `--local-buffer <dir>` (default `./localdata/local-buffer`) is where they land and `--local-buffer-max-size` (default 8GiB) bounds it; the stream is held once it fills. An empty `--local-buffer` turns it off, and so does `--with-constraints`.
...
- Sink: from-proto mode now runs without database constraints by default. `--no-constraints` is gone, replaced by `--with-constraints` (default false), so a fresh sink starts out on the fast path — no constraints, multi-row inserts, local buffer on — instead of needing a flag to get there.
  ...
  Combining it with an explicit `--local-buffer` or `--local-buffer-max-size` is an error rather than a silent downgrade: binary COPY loads one table at a time, so a child row can reach the server before its parent and an immediate foreign key rejects it.
