...
- CLI: the `substreams-sink-sql` binary is now part of the `substreams` CLI. `substreams sink sql {run,setup,create-user,generate-csv,inject-csv,tools}` covers the DatabaseChanges mode (`db_out` modules with a `schema.sql`), and `substreams sink from-proto` covers the relational-mappings mode (arbitrary protobuf output). Differences from the standalone binary: the connection string moved from a positional argument to `--dsn` (or the `SUBSTREAMS_SINK_SQL_DSN` environment variable) on every subcommand, metrics/pprof listeners are opt-in flags, and `sink sql tools cursor delete <module_hash>` now deletes only the given module's cursor (the standalone binary deleted all of them due to a bug).
