...
- Sink: `substreams sink postgres` in from-proto mode gained `--local-buffer <dir>`, which buffers rows on local disk in PostgreSQL's binary COPY wire format and loads them from a background goroutine, one segment per transaction. The stream no longer waits on the database, and blocks already downloaded survive a restart instead of being streamed — and paid for — twice. `--local-buffer-max-size` (default 16GiB) bounds the buffer; the stream is held once it fills. Requires `--no-constraints`.
