...
func TestBuildConfig(t *testing.T) {
	...
	tests := []testStruct{
		...
		{
			name:                      "dev store starts above init block with store range",
			storeInterval:             100,
			productionMode:            false,
			needsStores:               true,
			graphInitBlock:            0,
		...
		{
			name:                      "dev store starts above init block without store range",
			storeInterval:             100,
			productionMode:            false,
			needsStores:               true,
			graphInitBlock:            0,
			lowestStoreInitBlock:      800,
			resolvedStartBlock:        738,
			linearHandoffBlock:        738,
			exclusiveEndBlock:         742,
			expectStoresRange:         "nil",
		...
		{
			name:                      "dev store starts above init block without store range equal",
			storeInterval:             100,
			productionMode:            false,
			needsStores:               true,
			graphInitBlock:            0,
			lowestStoreInitBlock:      738,
			resolvedStartBlock:        738,
			linearHandoffBlock:        738,
			exclusiveEndBlock:         742,
			expectStoresRange:         "nil",
		...
		{
			name:                      "prod store starts above init block without store range equal",
			storeInterval:             100,
			productionMode:            true,
			needsStores:               true,
			graphInitBlock:            0,
			lowestStoreInitBlock:      810,
			resolvedStartBlock:        738,
			linearHandoffBlock:        800,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res, err := BuildTier1RequestPlan(tt.productionMode, uint64(tt.storeInterval), tt.graphInitBlock, tt.lowestStoreInitBlock, tt.resolvedStartBlock, tt.linearHandoffBlock, tt.exclusiveEndBlock, tt.needsStores)
			assert.Nil(t, err)
