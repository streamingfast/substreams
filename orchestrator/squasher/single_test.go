...
func TestShouldSaveFullKV(t *testing.T) {
	...
	for _, test := range tests {
		squasher := &Single{storeSaveInterval: test.storeSaveInterval}
...
func TestStoreSquasher_sortRange(t *testing.T) {
	s := &Single{files: store.PartialFiles("10-20,40-50,0-10")}
	s.sortRange()
	assert.Equal(t, store.PartialFiles("0-10,10-20,40-50"), s.files)
}

func TestStoreSquasher_ensureContiguity(t *testing.T) {
	s := &Single{files: store.PartialFiles("10-20,40-50,45-48")}
	assert.Error(t, s.ensureNoOverlap())

	s = &Single{files: store.PartialFiles("10-20,40-50")}
	assert.NoError(t, s.ensureNoOverlap())

	s = &Single{files: store.PartialFiles("10-20,20-50")}
	assert.NoError(t, s.ensureNoOverlap())
}
