...
func (p *Pipeline) backprocessStores(
	...
	for _, mod := range p.storeModules {
		splitWorks[mod.Name] = orchestrator.SplitSomeWork(mod.Name, uint64(p.blockRangeSizeSubRequests), mod.InitialBlock, uint64(p.request.StartBlockNum), storageState.Snapshots[mod.Name])
	}
