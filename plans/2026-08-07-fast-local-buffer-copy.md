> **Superseded.** The design below shipped, but its vocabulary did not: the local buffer
> is the spool, `--local-buffer*` became `--spool-*`, and the knobs it left hardcoded are
> now flags. See `plans/2026-08-12-coherent-sink-sql-flags.md` for the current shape.
