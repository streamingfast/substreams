...
// pendingDelivery is the block currently being delivered, kept on disk next to
// the cursor file. It exists from just before the first POST attempt until
// the cursor for that block has been written, so any interruption in between
// (a failed delivery in exit mode, a kill, a crash) is recovered on the next
// start by delivering it again before a Substreams stream is opened.
