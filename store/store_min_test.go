...
func TestStoreSetMinBigInt(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "found less",
			store:         NewTestKVStore(t, pbsubstreams.Module_KindStore_UPDATE_POLICY_UNSET, "", nil),
			key:           "key",
	...
	initTestStore := func(b *KVStore, key string, value *big.Int) {
...
func TestStoreSetMinInt64(t *testing.T) {
	...
	initTestStore := func(b *KVStore, key string, value *int64) {
		b.kv = map[string][]byte{}
		if value != nil {
...
func TestStoreSetMinFloat64(t *testing.T) {
	...
	initTestStore := func(b *KVStore, key string, value *float64) {
		b.kv = map[string][]byte{}
		if value != nil {
...
func TestStoreSetMinBigFloat(t *testing.T) {
	tests := []struct {
		name          string
		store         *KVStore
		key           string
	...
	initTestStore := func(b *KVStore, key string, value *big.Float) {
		b.kv = map[string][]byte{}
		if value != nil {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.store.SetMinBigFloat(0, test.key, test.value)
			actual, found := test.store.GetAt(0, test.key)
			if !found {
				t.Errorf("value not found")
			}
