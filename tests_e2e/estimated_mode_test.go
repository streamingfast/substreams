...
func TestEstimatedMode(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "estimate mode in development mode",
			startBlock:     1,
			stopBlock:      5,
			...
			for i, blockData := range blockScopedDataSlice {
				...
				if tc.estimateMode {
					if blockData.Output != nil {
						...
						if mapOutput != nil {
							// The MapOutput should be empty in estimate mode
							assert.Nil(t, mapOutput.MapOutput, "MapOutput should be empty in estimate mode for block %d", i)
						}
					}
				}
...
func TestEstimatedModeMetricsCollection(t *testing.T) {
	ctx := context.Background()
	zlog := zap.NewNop()
...
func TestEstimatedModeConcurrentRequests(t *testing.T) {
	ctx := context.Background()
	zlog := zap.NewNop()
	...
	for i := 0; i < numConcurrentRequests; i++ {
		go func(requestID int) {
			request := &pbsubstreamsrpcv2.Request{
				StartBlockNum:   int64(requestID + 1),
				StopBlockNum:    uint64(requestID + 2),
				FinalBlocksOnly: true,
				ProductionMode:  true,
				EstimateMode:    true,
				OutputModule:    "map_events",
