...
func checkE(cmd *cobra.Command, args []string) error {
	...
	files, err := stateStore.ListSnapshotFiles(ctx, 0, nil)
	if err != nil {
...
func newStore(storeURL string) (*store2.FullKV, dstore.Store, error) {
	...
	config, err := store2.NewConfig("", 0, "", pbsubstreams.Module_KindStore_UPDATE_POLICY_SET_IF_NOT_EXISTS, "", remoteStore, nil, 0)
	if err != nil {
		return nil, nil, err
	}
