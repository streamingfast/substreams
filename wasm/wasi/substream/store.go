...
func (s *bytesStore) GetAt(ord uint64, key string) ([]byte, error) {
	return os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/read/at/%d/%s", s.idx, ord, key),
	)
}

func (s *bytesStore) GetLast(key string) ([]byte, error) {
	return os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/read/last/%s", s.idx, key),
	)
}

func (s *bytesStore) GetFirst(key string) ([]byte, error) {
	return os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/read/first/%s", s.idx, key),
	)
}

func (s *bytesStore) HasAt(ord uint64, key string) bool {
	data, err := os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/check/at/%d/%s", s.idx, ord, key),
	)
	if err != nil {
		return false
	}
...
func (s *bytesStore) HasLast(key string) bool {
	data, err := os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/check/last/%s", s.idx, key),
	)
	if err != nil {
		return false
	}
...
func (s *bytesStore) HasFirst(key string) bool {
	data, err := os.ReadFile(
		fmt.Sprintf("/sys/stores/%d/check/first/%s", s.idx, key),
	)
	if err != nil {
		return false
	}
...
//todo: check this
func (s *bytesStore) Set(ord uint64, key string, value []byte) error {
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/write/at/%d/%s", s.idx, ord, key),
		value, 0644,
	)
}

//todo: check this
func (s *bytesStore) SetMany(ord uint64, keys []string, value []byte) error {
...
//todo: check this
func (s *bytesStore) SetIfNotExists(ord uint64, key string, value []byte) error {
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/write/ifnotexists/%d/%s", s.idx, ord, key),
		value, 0644,
	)
}

//todo: check this
func (s *bytesStore) SetIfNotExistsMany(ord uint64, keys []string, value []byte) error {
	for _, key := range keys {
		err := os.WriteFile(
			fmt.Sprintf("/sys/stores/%d/write/ifnotexists/%d/%s", s.idx, ord, key),
			value, 0644,
...
//todo: check this
func (s *bytesStore) DeletePrefix(ord uint64, prefix string) error {
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/delete/prefix/%d/%s", s.idx, ord, prefix),
		[]byte{}, 0644,
	)
}

func (s *bytesStore) into(data []byte) ([]byte, error) {
	return data, nil
}

func (s *bytesStore) from(item []byte) ([]byte, error) {
	return item, nil
}
...
type protoStore[M ProtoMarhsaler] struct {
	bytesStore *bytesStore //for some reason, it does not like to be embedded for this struct. perhaps because of the generic type?
}
...
func (s *protoStore[M]) Set(ord uint64, key string, value M) error {
	v, err := s.from(value)
	if err != nil {
		return fmt.Errorf("converting value %q: %w", value, err)
	}
...
func (s *stringStore) GetFirst(key string) (string, error) {
	data, err := s.bytesStore.GetFirst(key)
	if err != nil {
		return "", fmt.Errorf("reading last key %q: %w", key, err)
	}
	return s.into(data)
...
func (s *int64Store) Add(ord uint64, key string, value int64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/add/int64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *int64Store) Min(ord uint64, key string, value int64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/min/int64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *int64Store) Max(ord uint64, key string, value int64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/max/int64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *int64Store) into(data []byte) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("parsing int64 from string %q: %w", s, err)
	}
	return i, nil
}
...
func (s *float64Store) Add(ord uint64, key string, value float64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/add/float64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *float64Store) Min(ord uint64, key string, value float64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/min/float64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *float64Store) Max(ord uint64, key string, value float64) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/max/float64/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *float64Store) into(data []byte) (float64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("parsing float64 from string %q: %w", s, err)
	}
	return f, nil
}
...
func (s *bigIntStore) Add(ord uint64, key string, value *big.Int) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/add/bigint/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *bigIntStore) Min(ord uint64, key string, value *big.Int) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/min/bigint/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *bigIntStore) Max(ord uint64, key string, value *big.Int) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/max/bigint/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *bigFloatStore) Add(ord uint64, key string, value *big.Float) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/add/bigfloat/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *bigFloatStore) Min(ord uint64, key string, value *big.Float) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/min/bigfloat/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *bigFloatStore) Max(ord uint64, key string, value *big.Float) error {
	...
	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/max/bigfloat/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
...
func (s *arrayStore) Append(ord uint64, key string, item string) error {
	...
	if err != nil {
		return fmt.Errorf("converting value %q: %w", item, err)
	}

	return os.WriteFile(
		fmt.Sprintf("/sys/stores/%d/append/at/%d/%s", s.idx, ord, key),
		v, 0644,
	)
}
